...
{
    public class AjaxUploadHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        /// <summary>
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string uploadIframeId = app.Context.Request.QueryString["uploadIframeId"];
            if (UploadId != null) //AjaxUpload.aspx
            {
                ...
                if (sSourcePath.IndexOf("AjaxUpload.aspx") != -1)  //
                {
                    UploadFile(context);
                }
            }
            if (uploadIframeId != null) //Demo/uploadProgress.aspx
            {
                context.Response.Write(GetUploadProgressHtml());
            }
        }
        ...
        protected string GetUploadHtml(bool HasProgress)
        {
            ...
            sHtml.Append("<form name=\"form1\" method=\"post\" action=\"AjaxUpload.aspx\" id=\"form1\" enctype=\"multipart/form-data\">" + Environment.NewLine);
        ...
        protected string GetUploadProgressHtml()
        {
            return APTemplate.Properties.Resources.ResourceManager.GetString("UploadProgress");
        }
                ...
                else if (context.Request.Files.Count >= 1)
                {
                    for (int i = 0; i < context.Request.Files.Count; ++i)
                    {
                        ...
                        if (file.FileName != "")
                        {
                            ...
                            HttpContext.Current.Cache.Insert(sGuid + "_FileLength", file.ContentLength, null, DateTime.Now.AddMinutes(120), Cache.NoSlidingExpiration);
                            HttpContext.Current.Cache.Insert(sGuid + "_UploadFile", IsUseVirtualPath ? Path.Combine(context.Server.MapPath(@"~\" + UploadDir), FileInfo.Name) : Path.Combine(@UploadDir, FileInfo.Name), null, DateTime.Now.AddMinutes(120), Cache.NoSlidingExpiration);
                            ...
                            using (FileStream fs = new FileStream(Path.Combine(path, FileName), FileMode.Create, FileAccess.ReadWrite))
                            {
                                while (UploadedLength < file.ContentLength)
                                {
                                    ...
                                    HttpContext.Current.Cache.Insert(sGuid + "_CurrentFileLength", UploadedLength, null, DateTime.Now.AddMinutes(120), Cache.NoSlidingExpiration);
                                }
                            }
                        }
                        else
        ...
        private string GetScript(bool HasProgress)
        {
            ...
            sScript.Append("document.getElementById(\"form1\").action = \"AjaxUpload.aspx?id=" + UploadId + "\";" + Environment.NewLine);
        ...
        /// </summary>
        [AjaxMethod()]
        //public object doProgress(string Guid)
        public object doProgress(string SessionID)
        {
            try
            {
                var result = new { percent = -1, currentSize = 0.0d, size = 0.0d };
                if (HttpContext.Current.Cache.Get(SessionID + "_FileLength") == null)
                {
                    return result;
                }
                else
                {
                    int iTotalFileLength = (int)HttpContext.Current.Cache.Get(SessionID + "_FileLength");
                    if (iTotalFileLength == 0)
                    {
                        result = new { percent = iTotalFileLength, currentSize = 0.0d, size = 0.0d };
                        return result;
                    }
                    else
                    {
                        FileInfo FileInfo = new FileInfo((string)HttpContext.Current.Cache.Get(SessionID + "_UploadFile"));
                        if (!FileInfo.Exists) return result;
                        Double currentSizeMB = Math.Round(Convert.ToDouble(FileInfo.Length) / 1024 / 1024, 1);
                        Double sizeMB = Math.Round(Convert.ToDouble(HttpContext.Current.Cache.Get(SessionID + "_FileLength")) / 1024 / 1024, 1);
                        result = new { percent = (int)(FileInfo.Length * 100 / iTotalFileLength), currentSize = currentSizeMB, size = sizeMB };
                        return result;
                    }
                }
            }
